(function () {
    ...
    var onLoadAsync = async function () {
        const outputElement = document.querySelector("#bicep-whatif-reports-output");
        try {
            ...
            document.querySelector("#bicep-whatif-reports-filename").textContent = path.markdownPath();
    ...
    DOMPurify.addHook('beforeSanitizeAttributes', function (node) {
        if (node.tagName === 'A') {
            if (node.hasAttribute('target') && node.hasAttribute('rel') && node.getAttribute('rel') == 'noopener') {
