...
describe('Azure DevOps Extension - Integration & Edge Case Tests', function () {
    ...
    describe('Real-world Scenarios', function () {
        ...
        it('should handle mixed create, modify, and delete operations', async function () {
            ...
            try {
                ...
                const createChange = result.changes.find((c) => c.changeType === 'Create');
                const modifyChange = result.changes.find((c) => c.changeType === 'Modify');
                const deleteChange = result.changes.find((c) => c.changeType === 'Delete');
    ...
    describe('JSON Content Edge Cases', function () {
        it('should handle JSON with null values in expected places', async function () {
            ...
            try {
                const result = await (0, parseWhatIfJson_1.parseWhatIfJson)(testFile);
                assert.strictEqual(result.changes[0].after, null);
                assert.strictEqual(result.changes[0].delta, null);
                assert.strictEqual(result.error, null);
            }
            finally {
                if (fs.existsSync(testFile)) {
        ...
        it('should handle JSON with very large string values', async function () {
            ...
            try {
                const result = await (0, parseWhatIfJson_1.parseWhatIfJson)(testFile);
                assert.strictEqual(result.changes[0].after.properties.largeProperty.length, 10000);
                assert.strictEqual(result.changes[0].after.properties.description.length, 10000);
            }
            finally {
                if (fs.existsSync(testFile)) {
                    await fs.promises.unlink(testFile);
        ...
        it('should handle JSON with numeric precision edge cases', async function () {
            ...
            try {
                const result = await (0, parseWhatIfJson_1.parseWhatIfJson)(testFile);
                const props = result.changes[0].after.properties;
    ...
    describe('Error Recovery and Resilience', function () {
        ...
        it('should handle concurrent file access gracefully', async function () {
            ...
            try {
                ...
                const promises = Array.from({ length: 10 }, () => (0, parseWhatIfJson_1.parseWhatIfJson)(testFile));
