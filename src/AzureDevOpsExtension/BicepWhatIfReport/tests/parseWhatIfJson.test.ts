...
describe('Azure DevOps Extension - JSON Parser Tests', function () {
    ...
    describe('Valid JSON Parsing', function () {
        it('should parse empty what-if JSON successfully', async function () {
            ...
            assert.strictEqual(Array.isArray((result as any).diagnostics), true);
            assert.strictEqual((result as any).diagnostics.length, 0);
        });
        ...
        it('should parse minimal what-if JSON with single create change', async function () {
            ...
            assert.strictEqual((result as any).status, 'Succeeded');
            assert.strictEqual(Array.isArray((result as any).changes), true);
            assert.strictEqual((result as any).changes.length, 1);
            
            const change = (result as any).changes[0];
        ...
        it('should parse complex what-if JSON with all change types', async function () {
            ...
            assert.strictEqual((result as any).status, 'Succeeded');
            assert.strictEqual(Array.isArray((result as any).changes), true);
            assert.strictEqual((result as any).changes.length, 5);
            
            const changeTypes = (result as any).changes.map((c: any) => c.changeType);
            ...
            const unsupportedChange = (result as any).changes.find((c: any) => c.changeType === 'Unsupported');
        ...
        it('should parse what-if JSON with errors and diagnostics', async function () {
            ...
            assert.strictEqual(typeof result, 'object');
            assert.strictEqual((result as any).status, 'Failed');
            assert.strictEqual(Array.isArray((result as any).changes), true);
            assert.strictEqual((result as any).changes.length, 0);
            assert.strictEqual(Array.isArray((result as any).diagnostics), true);
            assert.strictEqual((result as any).diagnostics.length, 2);
            assert.strictEqual(typeof (result as any).error, 'object');
            assert.strictEqual((result as any).error.code, 'InvalidTemplate');
        });
        ...
        it('should parse actual complex what-if JSON from report.json', async function () {
            ...
            assert.strictEqual((result as any).status, 'Succeeded');
            assert.strictEqual(Array.isArray((result as any).changes), true);
            assert.strictEqual((result as any).changes.length > 0, true);
            assert.strictEqual(Array.isArray((result as any).diagnostics), true);
        });
    });
    ...
    describe('Error Handling', function () {
        it('should throw error for non-existent file', async function () {
            ...
            try {
                ...
                assert.strictEqual((error as Error).message.includes('does not exist'), true);
            }
        });
    ...
    describe('Structure Validation', function () {
        ...
        it('should preserve all properties in parsed object', async function () {
            ...
            assert.strictEqual(typeof result, 'object');
            assert.strictEqual((result as any).hasOwnProperty('changes'), true);
            assert.strictEqual((result as any).hasOwnProperty('diagnostics'), true);
            assert.strictEqual((result as any).hasOwnProperty('error'), true);
            assert.strictEqual((result as any).hasOwnProperty('status'), true);
            assert.strictEqual((result as any).hasOwnProperty('potentialChanges'), true);
        });
        ...
        it('should handle various change types correctly', async function () {
            ...
            // Test Modify change
            const modifyChange = changes.find((c: any) => c.changeType === 'Modify');
            assert.strictEqual(typeof modifyChange.before, 'object');
            assert.strictEqual(typeof modifyChange.after, 'object');
            assert.strictEqual(Array.isArray(modifyChange.delta), true);
            
            // Test NoChange
            const noChange = changes.find((c: any) => c.changeType === 'NoChange');
            ...
            // Test Ignore
            const ignoreChange = changes.find((c: any) => c.changeType === 'Ignore');
            assert.strictEqual(typeof ignoreChange.before, 'object');
            assert.strictEqual(typeof ignoreChange.after, 'object');
            
            // Test Unsupported
            const unsupportedChange = changes.find((c: any) => c.changeType === 'Unsupported');
            assert.strictEqual(typeof unsupportedChange.unsupportedReason, 'string');
        });
    });
    ...
    describe('Performance and Robustness', function () {
        ...
        it('should handle JSON with deeply nested structures', async function () {
            ...
            try {
                const result = await parseWhatIfJson(testFile);
                assert.strictEqual(typeof result, 'object');
                assert.strictEqual((result as any).changes[0].after.properties.virtualNetworkPeerings[0].properties.remoteVirtualNetwork.properties.subnets[0].properties.routeTable.properties.routes[0].properties.nextHopIpAddress, "10.0.0.1");
            } finally {
                // Clean up
                if (fs.existsSync(testFile)) {
                    await fs.promises.unlink(testFile);
